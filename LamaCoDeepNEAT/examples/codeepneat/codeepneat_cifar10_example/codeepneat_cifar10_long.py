...
import sys
sys.path.append('/storage/brno12-cerit/home/pivodovr/Tensorflow-Neuroevolution-master/')
import tfne
...
def codeepneat_cifar10_example(_):
    ...
    config_file_path = '/storage/brno12-cerit/home/pivodovr/Tensorflow-Neuroevolution-master/examples/codeepneat/codeepneat_cifar10_example/codeepneat_cifar10_long_config.cfg'
    
    backup_dir_str = datetime.now(tz=datetime.now().astimezone().tzinfo)
    backup_dir_str = backup_dir_str.strftime("%m-%d_%H-%M-%S_")
    backup_dir_str += str(random.randrange(99)) + "/"
    
    backup_dir_path = '/storage/brno12-cerit/home/pivodovr/results/final/cdn_long/' + backup_dir_str
    max_generations = 72
    max_fitness = None
    ...
    best_genome.save_genotype(save_dir_path=backup_dir_path+'best_genome_genotype/')
    best_genome.save_model(file_path=backup_dir_path+'best_genome_model/')
