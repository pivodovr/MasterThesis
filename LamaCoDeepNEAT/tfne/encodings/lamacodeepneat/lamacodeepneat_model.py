...
from tfne.encodings.codeepneat.codeepneat_model import CoDeepNEATModel

import numpy as np
from absl import logging


class LamaCoDeepNEATModel(CoDeepNEATModel):
    """
    ...
    def _create_model(self):
        ...
        # Create container for module weights, which will be after model creation assigned to model layers.
        # {layer, weights}
        layer_weights = {}
        ...
        for topology_level in graph_topology[1:]:
            for node in topology_level:
                ...
                for layer in node_layers:
                    ...
                    # Save module weights, which will be later assigned to whole model layers
                    if layer.__class__.__name__ == "Conv2D" and current_node_module.get_weights() is not None:
                        layer_weights[layer] = np.copy(current_node_module.get_weights())
        ...
        for layer in self.model.layers:
            if layer in layer_weights.keys():
                weights = layer_weights[layer]
                if weights[0].shape != layer.get_weights()[0].shape:
                    weights = [reshape_3rdweights(weights[0], layer.get_weights()[0].shape[2]), weights[1]]
                try:
                    layer.set_weights(weights)
                except ValueError:
                    logging.warning(f"Weight inheritance did not work."
                                    f"Layer k:{layer.kernel_size}, f:{layer.filters} weights shape "
                                    f"{layer.weights[0].shape} and "
                                    f"module weights shape {weights[0].shape}")


def reshape_3rdweights(weights, new_dim):
    """
    Helper function reshaping 3rd dimension of weights according to new shape. The weight size is increased by adding 0.
    """
    if new_dim < weights.shape[2]:
        # delete weights of last units
        weights = np.delete(weights, np.s_[new_dim:], 2)
    elif new_dim > weights.shape[2]:
        # add weights of last units
        input_to_add = np.zeros((weights.shape[0], weights.shape[1], new_dim - weights.shape[2], weights.shape[3]))
        weights = np.concatenate((weights, input_to_add), 2)
    return weights
