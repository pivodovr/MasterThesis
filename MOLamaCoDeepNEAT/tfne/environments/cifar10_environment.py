...
import tfne.algorithms.lamacodeepneat.lamacodeepneat
...
class CIFAR10Environment(BaseEnvironment):
    ...
    def __init__(self, weight_training, config=None, verbosity=0, lamarck=False, **kwargs):
        """
    ...
    def eval_genome_fitness(self, genome) -> (float, int):
        # TO BE OVERRIDEN
        raise RuntimeError()

    def _eval_genome_fitness_weight_training(self, genome) -> (float, int):
        """
        ...
        # Compile, train model and measure the training time
        model.compile(optimizer=optimizer, loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
                      metrics=[tf.metrics.SparseCategoricalAccuracy("accuracy")])
        history = model.fit(x=self.train_images,
                            y=self.train_labels,
                            epochs=self.epochs,
                            batch_size=self.batch_size,
                            verbose=self.verbosity)

        # If the LamaCoDeepNEAT is the running algorithm, then remember learned weights
        if self.lamarck:
            # If the trained weights are nan, genome gets minimal fitness
            if not tfne.algorithms.lamacodeepneat.lamacodeepneat.weights_not_nan(model.get_weights()):
                return 100, math.inf
            genome.set_weights(np.copy(model.get_weights()))

        # Determine fitness by creating model predictions with test images and then judging the fitness based on the
        # achieved model accuracy. Return this fitness as error rate.
        self.accuracy_metric.reset_states()
        self.accuracy_metric.update_state(self.squeezed_test_labels, np.argmax(model(self.test_images), axis=-1))
        fitness = 100 - round(self.accuracy_metric.result().numpy() * 100, 4)

        # Compute a flops value of genome and use it as computational complexity variable.
        flops = genome.compute_flops()
        return fitness, flops
