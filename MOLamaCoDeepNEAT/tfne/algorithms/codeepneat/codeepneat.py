    ...
    def evaluate_population(self, environment, save_dir_path) -> (int, float):
        ...
        for blueprint in self.pop.blueprints.values():
            ...
            for _ in range(self.genomes_per_bp):
                ...
                assigned_modules = dict()
                for i in bp_module_species:
                    chosen_module_id = random.choice(self.pop.mod_species[i])
                    assigned_modules[i] = self.pop.modules[chosen_module_id]
                # bp_id = blueprint.get_id()
                # bp_assigned_modules[bp_id] = assigned_modules
                ...
                try:
                    ...
                    # This genome is not added in the list of all generated genomes.
                    bp_id = blueprint.get_id()
                    mod_spec_to_id = dict()
                    for spec, mod in assigned_modules.items():
        ...
        genomes_pool = multiprocessing.pool.ThreadPool(1)
        ...
        for genome, genome_fitness in zip(generated_genomes, genomes_fitnesses):
            ...
            # To modules
            assigned_modules = genome.bp_assigned_modules
            for assigned_mod in assigned_modules.values():
